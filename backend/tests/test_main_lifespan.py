...
def test_lifespan_auto_index_ingest(monkeypatch: pytest.MonkeyPatch, tmp_path) -> None:
    ...
    monkeypatch.setattr(main.Base.metadata, "create_all", lambda bind=None: called.__setitem__("create_all", called["create_all"] + 1))
    monkeypatch.setattr(main.rag_store, "ingest_json_dir", lambda d: called.__setitem__("ingest", d))
    monkeypatch.setattr(main.rag_store, "rebuild_from_json_dir", lambda d: called.__setitem__("rebuild", d))
    ...
    monkeypatch.setenv("RAG_FAIL_FAST", "false")
...
def test_lifespan_fail_fast_re_raises(monkeypatch: pytest.MonkeyPatch, tmp_path) -> None:
    ...
    monkeypatch.setenv("RAG_DOCS_DIR", str(tmp_path))
    monkeypatch.setenv("RAG_REBUILD_ON_START", "false")
    monkeypatch.setenv("RAG_FAIL_FAST", "true")
